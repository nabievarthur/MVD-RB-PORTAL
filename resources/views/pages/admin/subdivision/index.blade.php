...
@section('content')
    <div class="flex w-full">
        <!-- Блок создания подразделения (1/3 ширины) -->
        <div class="w-1/3 bg-gray-800 rounded-lg shadow-md ml-4 mt-4 mb-4 p-4 overflow-auto">
            <h2 class="text-xl font-bold text-gray-200 text-center">Создание нового подразделения</h2>
            <form action="{{route('admin.subdivision.store')}}" method="POST" class="w-full">
                @csrf
                <div class="space-y-4">
        ...
        <div class="w-2/3 bg-gray-800 rounded-lg shadow-md m-4 p-4">
                ...
                <table class="w-full text-gray-300">
                    ...
                    @forelse($subdivisions as $subdivision)
                        <tr class="border-b border-gray-600 hover:bg-gray-600 transition-colors">
                            <td class="py-3 px-4">{{$subdivision->id}}</td>
                            <td class="py-3 px-4">{{$subdivision->title}}</td>
                            <td class="py-3 px-4 space-x-2 flex justify-center">
                                <a href="{{route('admin.subdivision.edit', $subdivision->id)}}"
                                ...
                                <button
                                    onclick="openModal('{{ route('admin.subdivision.destroy', $subdivision->id) }}', 'это подразделения')"
            ...
            <!-- Pagination -->
            <div class="mt-6">
                {{$subdivisions->withQueryString()->links()}}
            </div>
